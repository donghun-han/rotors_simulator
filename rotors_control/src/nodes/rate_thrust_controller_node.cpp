...
RateThrustControllerNode::RateThrustControllerNode() : nh_(""), pnh_("~") {
    InitializeParams();

    cmd_rate_thrust_sub_ = nh_.subscribe(kDefaultCommandRateThrustTopic, 1, &RateThrustControllerNode::RateThrustCallback, this);
    odometry_sub_ = nh_.subscribe(kDefaultOdometryTopic, 1, &RateThrustControllerNode::OdometryCallback, this);

    motor_velocity_reference_pub_ = nh_.advertise<mav_msgs::Actuators>(kDefaultCommandMotorSpeedTopic, 1);
}
...
void RateThrustControllerNode::InitializeParams() {
    ...
    GetRosParameter(pnh_, "angular_rate_gain/x",
                    rate_thrust_controller_.controller_parameters_.angular_rate_gain_.x(),
                    &rate_thrust_controller_.controller_parameters_.angular_rate_gain_.x());
    GetRosParameter(pnh_, "angular_rate_gain/y",
                    rate_thrust_controller_.controller_parameters_.angular_rate_gain_.y(),
                    &rate_thrust_controller_.controller_parameters_.angular_rate_gain_.y());
    GetRosParameter(pnh_, "angular_rate_gain/z",
                    rate_thrust_controller_.controller_parameters_.angular_rate_gain_.z(),
                    &rate_thrust_controller_.controller_parameters_.angular_rate_gain_.z());
    GetVehicleParameters(pnh_, &rate_thrust_controller_.vehicle_parameters_);
    rate_thrust_controller_.InitializeParameters();
}

void RateThrustControllerNode::RateThrustCallback(const mav_msgs::RateThrust::ConstPtr& rate_thrust_reference_msg) {
...
void RateThrustControllerNode::OdometryCallback(const nav_msgs::Odometry::ConstPtr& odometry_msg) {
