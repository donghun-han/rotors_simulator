...
void RateThrustController::CalculateRotorVelocities(Eigen::VectorXd* rotor_velocities) const {
    ...
    angular_rate_des[0] = rate_thrust_.angular_rates.x();
    angular_rate_des[1] = rate_thrust_.angular_rates.y();
    angular_rate_des[2] = rate_thrust_.angular_rates.z();

    Eigen::Vector3d angular_rate_error = odometry_.angular_velocity - angular_rate_des;

    angular_acceleration = - angular_rate_error.cwiseProduct(normalized_angular_rate_gain_) + odometry_.angular_velocity.cross(odometry_.angular_velocity);
